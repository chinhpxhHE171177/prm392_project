...
exports.getAttendance = async (req, res) => {
    try {
        const { employee_id, date, start_date, end_date, status, department_id, page, limit } = req.query;
        const user = req.user;
        ...
        if (date) {
            ...
            filters.date = dateMoment.format('YYYY-MM-DD'); // Đảm bảo định dạng ngày
        }
        if (start_date) filters.start_date = start_date;
        if (end_date) filters.end_date = end_date;
        if (status) filters.status = status;
...
// ... (Các phần khác của attendanceController.js giữ nguyên) ...
...
exports.checkIn = async (req, res) => {
    try {
        ...
        const user = req.user;
        const now = moment(); // Sử dụng thời gian local (GMT+7 nếu server đã cấu hình)
        const date = now.format('YYYY-MM-DD');
        const check_in_time = now.format('YYYY-MM-DD HH:mm:ss');

        console.log('Check-in request:', { employee_id, type, date, check_in_time });

        if (type !== 'checkin') {
            return res.status(400).json({ message: 'Loại yêu cầu không hợp lệ' });
        }

        console.log('Checking employee existence for employee_id:', employee_id);
        const employee = await checkEmployeeExistence(employee_id);
        if (!employee) {
            return res.status(400).json({ message: 'Nhân viên không tồn tại' });
        }
        ...
        if (employee.status === 'terminated') {
            return res.status(400).json({ message: 'Nhân viên đã nghỉ việc' });
        }
        if (employee.status === 'on_leave') {
            return res.status(400).json({ message: 'Nhân viên đang nghỉ phép' });
        }

        if (user.role === 'employee' && user.employee_id !== employee_id) {
            return res.status(403).json({ message: 'Không có quyền check-in cho nhân viên khác' });
        }
        if (!['admin', 'hr', 'dep_manager', 'employee'].includes(user.role)) {
            return res.status(403).json({ message: 'Không có quyền check-in' });
        }
        if (user.role === 'dep_manager') {
            console.log('Checking manager department for user.employee_id:', user.employee_id);
            const manager = await checkEmployeeExistence(user.employee_id);
            if (employee.department_id !== manager.department_id) {
                return res.status(403).json({ message: 'Không có quyền check-in cho nhân viên ngoài phòng ban' });
            }
        }
        ...
        if (attendanceDate.isAfter(currentDate)) {
            return res.status(400).json({ message: 'Không thể check-in cho ngày trong tương lai' });
        }
        if (user.role === 'employee' && !attendanceDate.isSame(currentDate)) {
            return res.status(400).json({ message: 'Nhân viên chỉ có thể check-in cho ngày hiện tại' });
        }

        console.log('Checking existing attendance for employee_id:', employee_id, 'date:', date);
        const existing = await Attendance.checkExisting(employee_id, date);
        console.log('Existing attendance:', existing);
        if (existing) {
            return res.status(400).json({ message: 'Bản ghi chấm công đã tồn tại cho ngày này' });
        }

        console.log('Fetching schedule for employee_id:', employee_id, 'date:', date);
        let schedule = await Attendance.getSchedule(employee_id, date);
        console.log('Schedule result:', schedule);
        if (!schedule) {
            console.warn(`No confirmed schedule found for employee_id ${employee_id} on ${date}. Using default schedule.`);
            schedule = {
                date,
        ...
        const status = calculateStatus(check_in_time, null, schedule);
        const attendanceData = { employee_id, date, check_in_time, status };
        ...
        if (['late', 'early_leave', 'absent'].includes(status)) {
            ...
            console.log('Fetching HR users for notification');
            const [hrUsers] = await db.execute(`SELECT id FROM users WHERE role = 'hr'`);
            ...
            if (hrUsers.length > 0) {
                ...
                console.log('Creating notifications:', notificationValues);
                if (notificationValues.length > 0) {
                    const placeholders = notificationValues.map(() => '(?, ?, ?, ?)').join(', ');
                    const flatValues = notificationValues.flat();
                    const query = `INSERT INTO notifications (user_id, title, message, type) VALUES ${placeholders}`;
                    await db.execute(query, flatValues);
                    }
                    else {
                    console.warn('No notifications created: notificationValues is empty');
                }
            } else {
                console.warn('No HR users found for notifications');
            }
        }

        return res.status(201).json({ message: 'Check-in thành công', status: 'success', attendance: newAttendance });
    } catch (error) {
...
exports.checkOut = async (req, res) => {
    try {
        const { employee_id, type } = req.body;
        const user = req.user;
        const now = moment(); // Sử dụng local time (GMT+7)
        const date = now.format('YYYY-MM-DD');
        const check_out_time = now.format('YYYY-MM-DD HH:mm:ss');

        console.log('Check-out request:', { employee_id, type, date, check_out_time });
        ...
        console.log('Existing attendance:', existing);
        if (!existing) {
            return res.status(400).json({ message: 'Không tìm thấy bản ghi check-in cho ngày này' });
        }

        if (user.role === 'employee' && user.employee_id !== employee_id) {
            return res.status(403).json({ message: 'Không có quyền check-out cho nhân viên khác' });
        }
        if (!['admin', 'hr', 'dep_manager', 'employee'].includes(user.role)) {
            return res.status(403).json({ message: 'Không có quyền check-out' });
        }
        if (user.role === 'dep_manager') {
            console.log('Checking manager department for user.employee_id:', user.employee_id);
            const manager = await checkEmployeeExistence(user.employee_id);
            if (employee.department_id !== manager.department_id) {
                return res.status(403).json({ message: 'Không có quyền check-out cho nhân viên ngoài phòng ban' });
            }
        }
        ...
        const attendance = await Attendance.findById(existing.id);
        if (moment(check_out_time).isBefore(moment(attendance.check_in_time))) {
            return res.status(400).json({ message: 'Thời gian check-out phải sau thời gian check-in' });
        }

        const status = calculateStatus(attendance.check_in_time, check_out_time, schedule);
        const updateData = { check_out_time, status };
        console.log('Updating attendance with data:', updateData);
        const isUpdated = await Attendance.update(existing.id, updateData);

        if (!isUpdated) {
            return res.status(400).json({ message: 'Không thể cập nhật check-out' });
        }

        const updatedAttendance = await Attendance.findById(existing.id);
        if (['late', 'early_leave', 'absent'].includes(status) && status !== attendance.status) {
            const employee = await checkEmployeeExistence(employee_id);
            const message = `Cập nhật chấm công ngày ${updatedAttendance.date}: ${status === 'late' ? 'Đến muộn' : status === 'early_leave' ? 'Rời sớm' : 'Vắng mặt'}`;
            await Attendance.createNotification(
                employee.user_id,
                'Cập Nhật Chấm Công',
                message,
        ...
        return res.status(200).json({ message: 'Check-out thành công', status: 'success', attendance: updatedAttendance });
    } catch (error) {
