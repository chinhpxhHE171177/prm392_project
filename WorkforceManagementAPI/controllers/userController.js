...
exports.updateProfile = async (req, res) => {
    try {
        ...
        if (userRole === 'admin') {
            // Admin chỉ có thể cập nhật username và email
            ...
            // Employee có thể cập nhật thông tin cá nhân
            const { username, email, first_name, last_name, phone, position } = updateData;

            // Cập nhật thông tin user
            if (username || email) {
                await User.updateUser(userId, { username, email });
            }

            // Cập nhật thông tin employee
            if (first_name || last_name || phone || position) {
                await User.updateEmployee(userId, {
                    first_name,
                    last_name,
                    phone,
                    position
...
exports.getUsersWithoutEmployee = async (req, res) => {
    try {
        ...
        console.log(`[${new Date().toISOString()}] getUsersWithoutEmployee called, users:`, users);
        return res.status(200).json({
            success: true,
            data: users,
            message: users.length > 0 
                ? 'Users without employee records fetched successfully'
                : 'No users without employee records found'
        });
    } catch (error) {
