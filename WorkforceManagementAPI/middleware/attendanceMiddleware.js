...
const checkInOutSchema = Joi.object({
    ...
    type: Joi.string().valid('checkin', 'checkout').required().messages({
        'any.only': 'Loại phải là một trong: checkin, checkout',
...
exports.checkInOutValidator = async (req, res, next) => {
    try {
        console.log('Validating check-in/check-out:', req.body);
        await checkInOutSchema.validateAsync(req.body, { abortEarly: false });
        next();
    } catch (error) {
        console.error('Validation errors:', error.details);
        return res.status(400).json({
