...
//    }
    ...
    private void handleSuccessfulLogin(LoginResponse response) {
        ...
        // Lưu userId vào PreferenceManager
        if (response != null && response.getData() != null) {
            int userId = response.getData().getId();
            new PreferenceManager(this).setUserId(userId);
        }
        ...
        loginViewModel.getProfile().observe(this, profileResponse -> {
            ...
            // Lưu employeeId nếu có, nếu không thì báo lỗi
            if (user != null && user.getProfileType() != null && user.getProfileType().equals("employee")) {
                if (user.getEmployeeId() != null) {
                    new PreferenceManager(this).setEmployeeId(user.getEmployeeId());
                    Log.d("Login", "Saved employeeId: " + user.getEmployeeId());
                    goToDashboard(user, profileResponse);
                } else {
                    Toast.makeText(this, "Không lấy được mã nhân viên. Vui lòng liên hệ quản trị.", Toast.LENGTH_LONG).show();
                    Log.e("Login", "employee_id not found in profile!");
                }
            } else {
                goToDashboard(user, profileResponse);
            }
    ...
    private void goToDashboard(User user, LoginResponse profileResponse) {
        Intent intent;
