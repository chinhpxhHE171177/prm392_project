...
public class TaskRepository {
    ...
    /**
     * Lấy danh sách task của user theo ngày hôm nay (lọc local).
     */
    public interface TaskListCallback {
        void onTasksLoaded(java.util.List<com.example.workforcemanagement.data.model.Task> tasks);
    }

    public void getTasksForToday(int assigneeId, String today, TaskListCallback callback) {
        // Lấy tất cả task của user này (page=1, limit=100)
        getTasks("", "", "", 1, 100).observeForever(tasksResponse -> {
            java.util.List<com.example.workforcemanagement.data.model.Task> result = new java.util.ArrayList<>();
            if (tasksResponse != null && tasksResponse.isSuccess() && tasksResponse.getData() != null) {
                for (com.example.workforcemanagement.data.model.Task task : tasksResponse.getData().getTasks()) {
                    if (task.getAssigneeId() != null && task.getAssigneeId() == assigneeId) {
                        // So sánh ngày (start_date hoặc deadline) với today
